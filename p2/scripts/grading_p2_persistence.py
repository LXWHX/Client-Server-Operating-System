...
expectsize += 4 + 4 + cse303.next4(len(alice.name) + 4 + 16 + 4 + 32 + 4)
cse303.after(
    server.pid
)  # need an extra cleanup to handle the KEY that was sent by first REG
cse303.do_cmd("Setting key b1.", "OK__", client.kvI(alice, "b1", fileb1), server)
expectsize += cse303.next4(4 + 4 + 4 + len("b1") + cse303.get_len(fileb1))
for i in range(40):
    key = "k" + str(i)
    file = key + ".dat"
    cse303.build_file(file, 64)
    cse303.do_cmd(
        "Setting key " + key + ".", "OK__", client.kvI(alice, key, file), server
    )
    expectsize += cse303.next4(4 + 4 + 4 + len(key) + cse303.get_len(file))
...
for i in range(40):
    key = "k" + str(i)
    file = key + ".dat"
    cse303.do_cmd("Getting key " + key + ".", "OK__", client.kvG(alice, key), server)
    cse303.check_file_result(file, key)
    cse303.delfile(file)
cse303.do_cmd("Getting key b1.", "OK__", client.kvG(alice, "b1"), server)
