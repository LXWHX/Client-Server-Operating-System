...
for i in range(1, 9):
    cse303.do_cmd(
        "Setting key k" + str(i) + " to 1K file.",
        "OK__",
        client.kvI(alice, "k" + str(i), t1kname),
        server,
    )
cse303.do_cmd(
    "Setting key k9 to 1K file.",
    "ERR_QUOTA_UP",
    client.kvI(alice, "k9", t1kname),
    server,
)
...
for i in range(1, 9):
    cse303.do_cmd(
        "Upserting key b" + str(i) + " to 1K file.",
        "OK_INSERT",
        client.kvU(bob, "b" + str(i), t1kname),
        server,
    )
cse303.do_cmd(
    "Upserting key b9 to 1K file.",
    "ERR_QUOTA_UP",
    client.kvU(bob, "b9", t1kname),
    server,
)
...
cse303.line()
print(
    "Test #4: Waiting resets quotas, which are then expended with a mix of operations"
)
cse303.line()
print("Waiting for " + server.qi + " seconds so that quotas can reset")
cse303.waitfor(int(server.qi) + 1)  # one extra, to play it safe...
cse303.do_cmd(
    "Inserting key c1 to 1K file.", "OK__", client.kvI(bob, "c1", t1kname), server
)
cse303.do_cmd(
    "Inserting key c2 to 1K file.", "OK__", client.kvI(bob, "c2", t1kname), server
)
cse303.do_cmd(
    "Inserting key c3 to 1K file.", "OK__", client.kvI(bob, "c3", t1kname), server
)
cse303.do_cmd(
    "Upserting key c4 to 1K file.", "OK_INSERT", client.kvU(bob, "c4", t1kname), server
)
cse303.do_cmd(
    "Upserting key c5 to 1K file.", "OK_INSERT", client.kvU(bob, "c5", t1kname), server
)
cse303.do_cmd(
    "Upserting key c6 to 1K file.", "OK_INSERT", client.kvU(bob, "c6", t1kname), server
)
cse303.do_cmd(
    "Upserting key b7 to 1K file.", "OK_UPDATE", client.kvU(bob, "b7", t1kname), server
)
cse303.do_cmd(
    "Upserting key b8 to 1K file.", "OK_UPDATE", client.kvU(bob, "b8", t1kname), server
)
cse303.do_cmd(
    "Upserting key b9 to 1K file.",
    "ERR_QUOTA_UP",
    client.kvU(bob, "c9", t1kname),
    server,
)
...
for i in range(1, 9):
    cse303.do_cmd(
        "Inserting key b" + str(i) + " to 1K file.",
        "ERR_KEY",
        client.kvI(bob, "b" + str(i), t1kname),
        server,
    )
cse303.do_cmd(
    "Inserting key b1 to 1K file.",
    "ERR_QUOTA_UP",
    client.kvI(bob, "b1", t1kname),
    server,
)
