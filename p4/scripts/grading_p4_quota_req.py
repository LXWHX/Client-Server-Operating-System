...
cse303.do_cmd(
    "Setting key k5 to 1K file.",
    "ERR_QUOTA_REQ",
    client.kvI(alice, "k5", t1kname),
    server,
)
...
for i in range(1, 5):
    cse303.do_cmd(
        "Getting key k" + str(i) + ".", "OK__", client.kvG(bob, "k" + str(i)), server
    )
...
for i in range(5, 9):
    cse303.do_cmd(
        "Upserting key k" + str(i) + ".",
        "OK_INSERT",
        client.kvU(bob, "k" + str(i), t1kname),
        server,
    )
cse303.do_cmd(
    "Upserting key k9.", "ERR_QUOTA_REQ", client.kvU(bob, "k9", t1kname), server
)
...
for i in range(1, 5):
    cse303.do_cmd(
        "Upserting key k" + str(i) + ".",
        "OK_UPDATE",
        client.kvU(alice, "k" + str(i), t1kname),
        server,
    )
cse303.do_cmd(
    "Upserting key k5.", "ERR_QUOTA_REQ", client.kvU(alice, "k5", t1kname), server
)
...
cse303.waitfor(int(server.qi) + 1)  # one extra, to play it safe...
for i in range(4):
    cse303.do_cmd("Getting all keys.", "OK__", client.kvA(bob, allkeys), server)
...
for i in range(1, 5):
    cse303.do_cmd(
        "Deleting key k" + str(i) + ".", "OK__", client.kvD(alice, "k" + str(i)), server
    )
...
cse303.line()
cse303.do_cmd("Inserting key k4.", "OK__", client.kvI(bob, "k4", t1kname), server)
cse303.do_cmd("Deleting key k5.", "OK__", client.kvD(bob, "k5"), server)
cse303.do_cmd("Upserting key k7.", "OK_UPDATE", client.kvU(bob, "k7", t1kname), server)
cse303.do_cmd("Getting key k6.", "OK__", client.kvG(bob, "k6"), server)
